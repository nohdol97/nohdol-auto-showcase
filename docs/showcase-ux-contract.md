...
- Public catalog and install routes never contain or validate raw installer codes, product keys, verifiers, private object URLs, distribution administrator controls, or direct installer URLs. The separately authenticated `/admin/radar/` route owns only public-business opportunity research and receives no distribution binding or authority.
