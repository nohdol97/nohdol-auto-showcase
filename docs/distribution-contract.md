...
Adding a valid catalog entry is sufficient to create both app-specific routes. Each entry declares a `defaultAssetId` that must reference one of its allowlisted installer assets; the install form selects that platform without changing the displayed asset order. The install page collects an application ID, asset ID, and program-specific authorization code. It posts them over HTTPS to a separate Worker and clears the code after each attempt. No raw code or verifier is emitted into Pages.
...
The public route contains only the user installation and in-app activation instructions. It has no administrator credential, route, issue form, key list, or revoke control, and it never sends a product key from browser JavaScript.
