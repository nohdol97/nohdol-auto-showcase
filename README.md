...
The public UI keeps `nohdol-clean` as its functional UX base and applies the `expressive-showcase` level of the separate Abalone brand layer: action teal, warm signature coral, bounded deep-ink fields, exact versioned icon assets, plain-Korean voice, and evidence hierarchy. Verified external workflow evidence is separated from deterministic no-integration demonstrations; reduced-motion visitors receive still posters instead of animated GIFs. The route hierarchy and responsive/accessibility behavior are recorded in [`docs/showcase-ux-contract.md`](docs/showcase-ux-contract.md).
